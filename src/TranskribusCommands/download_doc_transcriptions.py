...
if __name__ == '__main__':
    ...
    parser.add_option("--dest"  ,  dest='dest'  , action="store", type="string", help="specify the target directory")    
...
#         trp = json.load(open(options.trp, "rb",encoding='utf-8'))
        ...
        page_count = trp["md"]["nrOfPages"]
        pages = trp["pageList"]["pages"]
        doc_trp = trp
        docFolder = False
        for pg in pages:
            doc_trp["pageList"]["pages"] = [pg]
            transcripts = pg["tsList"]["transcripts"]

            traceln("- Downloading collection %s to folder %s, as specified by trp data"%(colid, os.path.abspath(destRoot)))
            if not options.docid:
                options.docid = trp["md"]["docId"]
                traceln(" read docId from TRP: docId = %s"%options.docid) 
            logging.basicConfig(level=logging.INFO)
            for tr in transcripts:
                #if tr["nrOfTranscribedLines"] > 1:
                doc_trp["pageList"]["pages"][0]["tsList"]["transcripts"] = [tr]
                destDir = destRoot + "/" + str(options.docid) + "." + str(pg["pageNr"]) + \
                          "."  + str(tr["timestamp"]) + "." + str(tr["tsId"])
                #if tr["status"] == "GT":
                #    destDir += "." + "GT"
                #elif "toolName" in tr:
                #    if tr["toolName"][0:11] != "CITlab HTR:":
                #        continue
                #    #destDir += "." + "HTR"
                ...
                col_ts, docFolder, lFileList = trnkbs2ds.download_document_by_trp( \
                                               colid, options.docid, destDir, trp, \
                                               bOverwrite=options.bForce, bNoImage=options.bNoImage)
                docFolder = True
        if docFolder:
            colFolder = docFolder #inaccurate, but fine for rest of code 
            print("colFolder =", colFolder)
    traceln("- Done")
